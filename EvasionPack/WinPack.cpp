...
//	//}
//
//	//pe.GetPeInfo(peinfo.FileBuffer, &peinfo);
//
//	//PEInfo dllPe;
//
//	//// 以不执行 DllMain 的方式加载模块到当前的内存中
//	//dllPe.FileBuffer = (POINTER_TYPE)LoadLibraryExA(DllNameTable[0].c_str(), NULL, DONT_RESOLVE_DLL_REFERENCES);
//	//pe.GetPeInfo(dllPe.FileBuffer, &dllPe);
//
//	//// 从 dll 中获取到 start 函数，并计算它的页内偏移(加载基址 + 区段基址 + 段内偏移)
//	//POINTER_TYPE Start = (POINTER_TYPE)GetProcAddress((HMODULE)dllPe.FileBuffer, "start");
//	//auto startOffset = Start - dllPe.FileBuffer - GetSection(dllPe.FileBuffer, ".text")->VirtualAddress;
//
//	////添加新节存
//	//pe.AddSection(peinfo.FileBuffer, dllPe.FileBuffer, &peinfo);
//	//pe.GetPeInfo(peinfo.FileBuffer, &peinfo);
//
//	//// 新的 rav = start 的段内偏移 + 新区段的 rva
//	//GET_OPTIONAL_HEADER(peinfo.FileBuffer)->AddressOfEntryPoint = startOffset +
//	//	GetSection(peinfo.FileBuffer, ".vmp0")->VirtualAddress;
//
//	//// 获取到共享信息
//	//ShareData = (PSHAREDATA)GetProcAddress((HMODULE)dllPe.FileBuffer, "ShareData");
//
//	//ShareData->OldOep = peinfo.AddressOfEntryPoint;
//
//	////修复重定位表
//	//pe.FixReloc(peinfo.FileBuffer, dllPe.FileBuffer);
//
//	//pe.CopySectionData(peinfo.FileBuffer, dllPe.FileBuffer);
//
//	//pe.SaveFile(peinfo);
//
